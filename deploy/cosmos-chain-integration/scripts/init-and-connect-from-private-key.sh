...
VALIDATOR_PUB_KEY=$($GRAVITY $GRAVITY_HOME_FLAG tendermint show-validator)
echo "validator pub key: $VALIDATOR_PUB_KEY"

echo "Creating gentxs"
$GRAVITY $GRAVITY_HOME_FLAG gentx --pubkey $VALIDATOR_PUB_KEY --ip $GRAVITY_HOST $GRAVITY_VALIDATOR_NAME 100000000000$STAKE_DENOM "$ETH_ORCHESTRATOR_VALIDATOR_ADDRESS" "$(jq -r .address $GRAVITY_HOME/orchestrator_key.json)" $GRAVITY_KEYRING_FLAG $GRAVITY_CHAINID_FLAG
